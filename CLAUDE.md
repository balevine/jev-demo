...
- **Labeling is additive.** Only ever send `addLabelIds`. Never create a label and never remove one. Sending only `addLabelIds` is not enough on its own, because Gmail accepts its own label IDs there too and adding `TRASH` bins the thread. A write has to name labels the user made, so `/apply` checks the IDs it was given against `labels.list` before it writes. The classify path gets its IDs from `list_labels`, which already drops everything that is not a user label.
- **The server answers to loopback names only.** It can read and write a mailbox and it asks nobody for a password, so binding to 127.0.0.1 is half the job. A page in a browser can point a name it owns at 127.0.0.1 and reach the API as same origin, which no CORS rule would stop, so the middleware in `main.py` checks the Host header against `JEV_DEMO_ALLOWED_HOSTS` and turns away anything else. It reads that setting per request, which is what lets a test say which names count.
